...
export function NewNoteCard({ onNoteCreate }: NewNoteCardProps) {
  ...
  const contentRef = useRef<HTMLTextAreaElement>(null); // Ref to content textarea
  ...
  function handleCloseDialog() {
    setContent('');
    setShouldShowOnBoarding(true);
    handleStopRecording(); // Stop any ongoing recording

    if (contentRef.current) {
      contentRef.current.value = ''; // Clear textarea content explicitly
    }
  }
    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
          <Dialog.Close onClick={handleCloseDialog} className="absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100 transition-colors">
            <X className="size-5" />
          </Dialog.Close>
